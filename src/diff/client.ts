...
export interface DiffClient {
  /** The uncommitted changes of the tree `worktree` is in: staged, unstaged and untracked. */
  files(worktree: string): Promise<ChangeList>
  /** Both sides of one of them; `oldPath` for a renamed file. */
  sides(worktree: string, file: string, oldPath: string | null): Promise<FileSides>
}

export const tauriDiff: DiffClient = {
  files: (worktree) => invoke<ChangeList>('worktree_diff_files', { worktree }),
  sides: (worktree, file, oldPath) => invoke<FileSides>('worktree_diff_file', { worktree, file, oldPath }),
}
